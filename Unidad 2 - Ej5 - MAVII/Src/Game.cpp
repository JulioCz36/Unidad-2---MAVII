...
void Game::DrawGame()
{
    ...
    plano.setRotation(planoInclinado->GetAngle() * 180.0f / b2_pi);
    plano.setPosition(planoInclinado->GetPosition().x, planoInclinado->GetPosition().y);
    wnd->draw(plano);
...
void Game::InitPhysics()
{
    ...
    // Crear plano
    planoInclinado = Box2DHelper::CreateRectangularDynamicBody(phyWorld, 100, 10, 0.0f, friccion, 0.0);
    planoInclinado->SetTransform(b2Vec2(50.0f, 50.0f), -alpha);

    // Crear una caja
    cajaBody = Box2DHelper::CreateRectangularDynamicBody(phyWorld, 15.0f, 10.0f, 0.3f, friccion, 0.0);
    cajaBody->SetTransform(b2Vec2(50.0f, 0.0f), -alpha);
}
